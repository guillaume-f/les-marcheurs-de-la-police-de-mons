...
export type StrapiRequestParamPopulate<T> = {
    [K in keyof T]: T[K] extends object ? T[K] extends Array<infer I> ? `${Exclude<K, symbol>}` | `${Exclude<K, symbol>}.${StrapiRequestParamPopulate<I>}` : `${Exclude<K, symbol>}` | `${Exclude<K, symbol>}.${StrapiRequestParamPopulate<T[K]>}` : never;
}[keyof T];
export type StrapiPrimitiveOperators<T> = {
    ...
    $or: StrapiFilterRequestParam<T>[];
} | {
    $and: StrapiFilterRequestParam<T>[];
} | {
    $not: StrapiFilterRequestParam<T>;
};
...
export type StrapiFilterRequestParam<T> = T extends object ? {
    [K in keyof T]?: StrapiFilterRequestParam<T[K]>;
} : StrapiPrimitiveOperators<T> | (T extends string ? StrapiStringOperators : T extends number ? StrapiNumberOperators : never);
export * from './v3.js';
export * from './v4.js';
export * from './v5.js';
